...
const navItems = [
  {id:'home',icon:'🏠',label:'Home',to:'/'},{id:'diary',icon:'📔',label:'Diary',to:'/gallery'},
...
const settingsGrid = [
  ...
  [{id:'backup',icon:'💾',label:'Backup',desc:'Save data',color:T.onSurfaceVariant},
...
export default function MyPage() {
  ...
  function handleLogout(){setLoggingOut(true);localStorage.removeItem('room_code');window.location.reload()}

  const statItems=[{icon:'📔',val:stats.diary||142,label:'Diaries'},{icon:'📷',val:stats.photos,label:'Photos'},{icon:'🗺',val:stats.footprints,label:'Footprints'},{icon:'💝',val:stats.years,label:'Years'}]
      ...
      <div style={{ width:'100%',maxWidth:430,margin:'0 auto',position:'relative',zIndex:10 }}>
        ...
        <section onClick={()=>setShowEdit(true)} style={{ margin:'8px 20px',background:'rgba(255,255,255,0.6)',...T.glassLg,border:'1px solid rgba(255,255,255,0.6)',borderRadius:24,padding:20,display:'flex',alignItems:'center',gap:16,position:'relative',cursor:'pointer',transition:'box-shadow .2s' }}>
          ...
          <div style={{ flex:1,position:'relative',zIndex:1 }}>
            ...
            <span style={{ display:'inline-flex',alignItems:'center',gap:4,marginTop:4,padding:'4px 12px',borderRadius:999,background:'rgba(255,255,255,0.6)',color:T.onSurfaceVariant,fontSize:12,fontWeight:600,fontFamily:'"Plus Jakarta Sans",sans-serif' }}>♥ Couple Mode</span>
          </div>
        ...
        {/* ═══ Premium Member Card - bg-primary/20 backdrop-blur-lg rounded-[20px] ═══ */}
        <section style={{ margin:'0 20px 16px',background:'rgba(156,66,51,0.2)',...T.glassMd,border:'1px solid rgba(255,255,255,0.5)',borderRadius:20,padding:'16px 20px',display:'flex',alignItems:'center',justifyContent:'space-between' }}>
          <div>
            <div style={{ ...T.headlineMd,color:T.onBg,margin:0 }}>Premium Member</div>
            <div style={{ ...T.bodySm,color:T.onSurfaceVariant,marginTop:4 }}>Unlock all scrapbook features</div>
          </div>
        ...
        {/* ═══ Logout btn ═══ */}
        <button onClick={()=>setShowLogout(true)} style={{ margin:'0 20px 20px',width:'calc(100% - 40px)',padding:'14px',borderRadius:20,background:'rgba(186,26,26,0.08)',border:'1px solid rgba(186,26,26,0.2)',color:T.error,fontFamily:'"Plus Jakarta Sans",sans-serif',fontSize:14,fontWeight:600,cursor:'pointer' }}>
          🚪 Logout
        </button>
      </div>
      ...
      {showEdit&&(
        <div style={{ position:'fixed',inset:0,zIndex:100,display:'flex',alignItems:'center',justifyContent:'center',padding:20 }}>
          ...
          <div style={{ position:'relative',zIndex:1,width:'100%',maxWidth:360,background:T.surfContLowest,borderRadius:24,padding:'24px 18px',boxShadow:'0 16px 48px rgba(60,20,15,0.22)',animation:'modalIn .25s ease-out' }}>
            <h3 style={{ ...T.headlineMd,color:T.onBg,textAlign:'center',margin:'0 0 20px' }}>Edit Profile</h3>
            ...
            <button onClick={saveProfile} style={{ width:'100%',padding:14,borderRadius:18,background:T.primary,color:T.onPrimary,border:'none',fontSize:15,fontWeight:600,cursor:'pointer',fontFamily:'"Plus Jakarta Sans",sans-serif' }}>Save</button>
          </div>
        </div>
      )}
      ...
      {showLogout&&(
        <div style={{ position:'fixed',inset:0,zIndex:100,display:'flex',alignItems:'center',justifyContent:'center',padding:20 }}>
          ...
          <div style={{ position:'relative',zIndex:1,width:'100%',maxWidth:320,background:T.surfContLowest,borderRadius:24,padding:'28px 22px',textAlign:'center',boxShadow:'0 16px 48px rgba(60,20,15,0.22)',animation:'modalIn .25s ease-out' }}>
            ...
            <h3 style={{ ...T.headlineMd,color:T.onBg,fontStyle:'italic',margin:'0 0 8px' }}>Leave this scrapbook?</h3>
            <p style={{ ...T.bodySm,color:T.onSurfaceVariant,margin:'0 0 20px',fontFamily:'"Plus Jakarta Sans",sans-serif',lineHeight:1.6 }}>You can always come back with the same room code.</p>
            <div style={{ display:'flex',gap:10 }}>
              <button onClick={()=>setShowLogout(false)} disabled={loggingOut} style={{ flex:1,padding:12,borderRadius:16,background:'transparent',border:`1.5px solid ${T.outlineVariant}`,opacity:.4,color:T.onSurfaceVariant,cursor:'pointer',fontSize:14,fontFamily:'"Plus Jakarta Sans",sans-serif' }}>Cancel</button>
              <button onClick={handleLogout} disabled={loggingOut} style={{ flex:1,padding:12,borderRadius:16,background:T.error,color:T.onError,border:'none',cursor:'pointer',fontSize:14,fontWeight:600,fontFamily:'"Plus Jakarta Sans",sans-serif',opacity:loggingOut?.7:1 }}>{loggingOut?'Leaving...':'Leave'}</button>
            </div>
          </div>
        </div>
      )}
